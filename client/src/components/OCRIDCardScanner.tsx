...
import { useOCR, IDCardData } from "@/hooks/useOCR";
...
import { CreditCard, Upload, CheckCircle, AlertCircle, Camera, RefreshCw, Scan } from "lucide-react";
import { toast } from "sonner";

interface OCRIDCardScannerProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (data: IDCardData) => void;
}

export function OCRIDCardScanner({ open, onClose, onConfirm }: OCRIDCardScannerProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { isProcessing, progress, progressMessage, result, error, processImage, reset } = useOCR();
  const [preview, setPreview] = useState<string | null>(null);
  const [editData, setEditData] = useState<IDCardData | null>(null);
  ...
  const handleFileSelect = async (file: File) => {
    ...
    setPreview(url);
    setEditData(null);
    await processImage(file, "idcard");
  };
  ...
  if (result?.idCard && !editData) {
    setEditData({ ...result.idCard });
  }

  const handleConfirm = () => {
    if (!editData) return;
    onConfirm(editData);
    toast.success("บันทึกข้อมูลบัตรประชาชนเรียบร้อย");
    handleReset();
    onClose();
  };
      ...
      <DialogContent className="max-w-lg bg-[#0d1117] border border-white/10 text-white">
        <DialogHeader>
          ...
          <DialogDescription className="text-white/50">
            อัปโหลดรูปบัตรประชาชนเพื่ออ่านข้อมูลอัตโนมัติ
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {!preview && (
            <div
              ...
              className="border-2 border-dashed border-white/20 rounded-xl p-8 text-center cursor-pointer hover:border-cyan-500/50 hover:bg-cyan-500/5 transition-all duration-200 group"
            >
              <div className="flex flex-col items-center gap-3">
                ...
                <div>
                  <p className="text-white/80 font-medium">วางรูปที่นี่ หรือคลิกเพื่อเลือก</p>
                  <p className="text-white/40 text-sm mt-1">รองรับ JPG, PNG, WEBP</p>
                </div>
          ...
          {preview && (
            <div className="relative rounded-xl overflow-hidden border border-white/10">
              <img src={preview} alt="บัตรประชาชน" className="w-full max-h-48 object-contain bg-black/40" />
              {isProcessing && (
                <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
                  <div className="text-center space-y-2 px-4">
                    <div className="w-8 h-8 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin mx-auto" />
                    <p className="text-cyan-400 text-sm font-medium">{progressMessage}</p>
                  </div>
                </div>
              )}
              <button
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
                <div className="space-y-1">
                  <Label className="text-xs text-white/50">เลขบัตรประชาชน</Label>
                  <Input
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label className="text-xs text-white/50">ชื่อ</Label>
                    <Input
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs text-white/50">นามสกุล</Label>
                    <Input
                ...
                <div className="space-y-1">
                  <Label className="text-xs text-white/50">วันเดือนปีเกิด</Label>
                  <Input
              ...
              <p className="text-xs text-white/40 text-center">
                ตรวจสอบและแก้ไขข้อมูลก่อนกดยืนยัน
              </p>
